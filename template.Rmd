...
```{r fig1_data2, echo=FALSE, fig.height=3, fig.width=5, fig.cap="**Le Titre de ma figure**. Les éléments complémentaires pour comprendre ma figure (par exemple l'explication des abbréviations utilisées, le nombre d'individus surlequel porte les moyennes/écart-types, ...). Parfois tout un paragraphe... qui peut être très long et qui permet à la figure d'être *auto-portante*..."}
...
g.basic <- g + 
  geom_errorbar(width=.25, aes(ymin=direction-ecarttype, ymax=direction),  # ajout des écart-types
...
*"On observe sur la Figure 1 que, en absence de NPA, le pivotement de la boîte à 90° provoque une courbure moyenne de `r format(data2.summarized[data2.summarized$Position == "Courbe" & data2.summarized$NPA == "Sans", "direction"], digits = 0)`°, mesurée après 24h, alors qu'elle est moindre (`r format(data2.summarized[data2.summarized$Position == "Debout" & data2.summarized$NPA == "Sans", "direction"], digits = 0)`°), comme attendu, dans la condition témoin. L'ajout de NPA dans le milieu inhibe  cette courbure qui n'est plus que de `r format(data2.summarized[data2.summarized$Position == "Courbe" & data2.summarized$NPA == "Avec", "direction"], digits = 0)`° pour les plantes subissant le stimulus gravitropique et `r format(data2.summarized[data2.summarized$Position == "Debout" & data2.summarized$NPA == "Avec", "direction"], digits = 0)`° pour les témoins."*
...
```{r fig2_data1, echo=FALSE, fig.height=3, fig.width=5, fig.cap="**Le Titre de ma figure**. Les éléments complémentaires pour comprendre ma figure (par exemple l'explication des abbréviations utilisées, le nombre d'individus surlequel porte les moyennes/écart-types, ...). Parfois tout un paragraphe... qui peut être très long et qui permet à la figure d'être *auto-portante*..."}
# Définition des paramètres principaux du graphique
...
```{r fig3_data1, echo=FALSE, fig.height=3, fig.width=5, fig.cap="**Le Titre de ma figure**. Les éléments complémentaires pour comprendre ma figure (par exemple l'explication des abbréviations utilisées, le nombre d'individus surlequel porte les moyennes/écart-types, ...). Parfois tout un paragraphe... qui peut être très long et qui permet à la figure d'être *auto-portante*..."}
...
```{r fig1_data3, echo=FALSE, fig.height=3, fig.width=5, fig.cap="**Le Titre de ma figure**. Les éléments complémentaires pour comprendre ma figure (par exemple l'explication des abbréviations utilisées, le nombre d'individus surlequel porte les moyennes/écart-types, ...). Parfois tout un paragraphe... qui peut être très long et qui permet à la figure d'être *auto-portante*..."}
...
```{r fig1_data4, echo=FALSE, fig.height=3, fig.width=5, fig.cap="**Le Titre de ma figure**. Les éléments complémentaires pour comprendre ma figure (par exemple l'explication des abbréviations utilisées, le nombre d'individus surlequel porte les moyennes/écart-types, ...). Parfois tout un paragraphe... qui peut être très long et qui permet à la figure d'être *auto-portante*..."}
