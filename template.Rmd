...
data1.summarized <- data1[, j=list(diff=longueur[jour==1]-longueur[jour==0]), by=list(plante, lumière)][, list(mean=mean(diff), sd=sd(diff)), by=lumière]
head(data1.summarized)
```
...
bending <- data2[, list(diff.angle=angle[time==1]-angle[time==0]), by=list(Position, NPA, Plante)][, list(mean=mean(diff.angle), sd=sd(diff.angle)), by=list(Position,NPA)]
