...
L'introduction contient les éléments, étayés par la littératures scientifiques, qui supportent la ou les hypothèses que vous allez évaluer par votre expérimentation. A ce titre, toute information doit être reliée à sa source, c'est à dire à une référence bibliographique.

Ceci est une référence à 2 publications [@folta_light_2008; @symons_interactions_2003]

# Matériel et méthodes

Cette section décrit les conditions de votre expériementation, ainsi que le matériel utilisé. L'idée générale est que, sur bases des informations fournies, il soit possible de reproduire exactement votre expérience.
...
```{r fig1_data1, echo=FALSE, fig.height=3, fig.width=5, fig.cap="**Le Titre de ma figure 1**. Les éléments complémentaires pour comprendre ma figure 1. Parfois tout un paragraphe... qui peut être très long est qui permet à la figure d'être *auto-portante*..."}
...
```{r fig2_data1, echo=FALSE, fig.height=3, fig.width=5, fig.cap="**Le Titre de ma figure 1**. Les éléments complémentaires pour comprendre ma figure 1. Parfois tout un paragraphe... qui peut être très long est qui permet à la figure d'être *auto-portante*..."}
# Mise en forme des données
...
*"On observe sur la Figure 1 que, en absence de NPA, le pivotement de la boîte à 90° provoque une courbure moyenne de `r bending[bending$position == "Courbe" & bending$npa == "Sans", "mean"]`°, mesurée après 24h, alors qu'elle est quasi nulle (`r bending[bending$position == "Debout" & bending$npa == "Sans", "mean"]`°), comme attendu, dans la condition témoin. L'ajout de NPA dans le milieu inhibe considérablement cette courbure qui n'est plus que de `r bending[bending$position == "Courbe" & bending$npa == "Avec", "mean"]`° pour les plantes subissant le stimulus gravitropique et `r bending[bending$position == "Debout" & bending$npa == "Avec", "mean"]`° pour les témoins."*
